-- =====================================================
-- 🔍 DIAGNÓSTICO COMPLETO SUPABASE - MedCannLab 5.0 (CORRIGIDO)
-- =====================================================
-- Data: 06/02/2026
-- Estrutura Real Confirmada:
-- - chat_participants: NÃO tem created_at
-- - clinical_assessments: USA doctor_id (NÃO professional_id)
-- - clinical_reports: TEM professional_id E doctor_id
-- - appointments: TEM professional_id E doctor_id
...
-- Vínculos via clinical_assessments (USA doctor_id)
SELECT 
...
-- Vínculos via clinical_reports (TEM professional_id E doctor_id)
SELECT 
    cr.id as report_id,
    cr.patient_id,
    pu.name as patient_name,
...
-- Vínculos via appointments (TEM professional_id E doctor_id)
SELECT 
    a.id as appointment_id,
    a.patient_id,
    pu.name as patient_name,
    pu.email as patient_email,
    COALESCE(a.professional_id, a.doctor_id) as professional_id,
    pro.name as professional_name,
    pro.email as professional_email,
    a.appointment_date,
    a.status
...
LEFT JOIN public.users pro ON pro.id = COALESCE(a.professional_id, a.doctor_id)
ORDER BY a.appointment_date DESC
LIMIT 50;

-- Vínculos via chat_participants (SEM created_at)
SELECT 
    cp.room_id,
