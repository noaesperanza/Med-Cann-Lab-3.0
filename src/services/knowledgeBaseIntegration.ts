...
// GOVERNANÇA: A Base de Conhecimento usa APENAS documentos da biblioteca (tabela documents).
// Prontuários (patient_medical_records) NUNCA entram aqui — ficam restritos por RLS aos profissionais
// vinculados ao paciente (is_professional_patient_link). Não expor dados de pacientes na base de conhecimento.
...
export class KnowledgeBaseIntegration {
  private static stripInjectedContext(value: string): string {
    const raw = value || ''
  ...
  private static sanitizePostgrestOrTerm(value: string): string {
    // Evita quebrar o parser do PostgREST `.or(...)` e previne que contexto/JSON polua a busca.
    const safe = this.stripInjectedContext(value || '')
      .replace(/\r?\n+/g, ' ')
      .replace(/[,*()]/g, ' ') // remove separadores e curingas que quebram `.or()`
      .replace(/["'\\]/g, ' ')
      .replace(/\s+/g, ' ')
      .trim()

    // Limite de tamanho para evitar query gigantesca
    return safe.slice(0, 80)
  }

  /**
   * Buscar todos os documentos da base de conhecimento da IA
   */
  ...
  static async semanticSearch(
    ...
    try {
      ...
      const searchTerm = this.sanitizePostgrestOrTerm(query).toLowerCase()
