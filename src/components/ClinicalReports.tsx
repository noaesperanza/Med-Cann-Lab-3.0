    ...
    try {
      ...
      if (!rpcError && rpcReports) {
        ...
        const formattedReports: SharedReport[] = rpcReports.map((report: any) => {
          const rawContent = (typeof report.content === 'object' && report.content !== null) ? report.content : {}
          const defaultRationalities = {
            biomedical: {},
            traditionalChinese: {},
            ayurvedic: {},
            homeopathic: {},
          ...
          return {
            ...
            content: {
              chiefComplaint: rawContent.chiefComplaint || rawContent.chief_complaint || '',
              history: rawContent.history || '',
              physicalExam: rawContent.physicalExam || rawContent.physical_exam || '',
              assessment: rawContent.assessment || '',
              plan: rawContent.plan || '',
              rationalities: (typeof rawContent.rationalities === 'object' && rawContent.rationalities !== null)
                ? rawContent.rationalities
                : defaultRationalities
            },
            doctorNotes: rawContent.doctor_notes,
  ...
  const handleApplyRationality = async (rationality: Rationality) => {
    ...
    try {
      ...
      setSelectedReport({
        ...selectedReport,
        content: {
          ...(selectedReport.content ?? { chiefComplaint: '', history: '', physicalExam: '', assessment: '', plan: '', rationalities: updatedRationalities }),
            ...
            <div key={report.id} className="bg-white rounded-xl p-6 shadow-lg border border-slate-200 hover:shadow-xl transition-shadow">
              ...
              <div className="mb-4">
                ...
                <p className="text-slate-600 text-sm mb-2">
                  <strong>Queixa Principal:</strong> {report.content?.chiefComplaint || 'Não informada'}
                </p>
                <p className="text-slate-600 text-sm">
                  <strong>Avaliação:</strong> {report.content?.assessment || 'Não informada'}
                </p>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {Object.entries((report.content && typeof report.content.rationalities === 'object' && report.content.rationalities) || {}).map(([key, value]: [string, any]) => {
              ...
              <div className="flex items-center space-x-3">
                <button
                  onClick={() => {
                    ...
                    Object.entries((report.content && typeof report.content.rationalities === 'object' && report.content.rationalities) || {}).forEach(([key, value]: [string, any]) => {
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 rounded-lg p-4">
                ...
                <div className="space-y-2 text-sm">
                  <div><strong>Queixa Principal:</strong> {selectedReport.content?.chiefComplaint || 'Não informada'}</div>
                  <div><strong>História:</strong> {selectedReport.content?.history || 'Não informada'}</div>
                  <div><strong>Exame Físico:</strong> {selectedReport.content?.physicalExam || 'Não informado'}</div>
                  <div><strong>Avaliação:</strong> {selectedReport.content?.assessment || 'Não informada'}</div>
                  <div><strong>Plano:</strong> {selectedReport.content?.plan || 'Não informado'}</div>
                </div>
              </div>
              ...
              {selectedReport && Object.entries((selectedReport.content && typeof selectedReport.content.rationalities === 'object' && selectedReport.content.rationalities) || {}).some(([_, value]: [string, any]) => value && value.assessment) && (
                <div className="bg-slate-50 rounded-lg p-4 border border-slate-200">
                  <h4 className="font-semibold text-slate-800 mb-3">Análises por Racionalidade:</h4>
                  <div className="space-y-4">
                    {Object.entries((selectedReport.content && typeof selectedReport.content.rationalities === 'object' && selectedReport.content.rationalities) || {}).map(([key, value]: [string, any]) => {
